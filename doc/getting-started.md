...
This tutorial will help you to go through the basics to use Dogecoin Core after you completed the [installation instructions](/INSTALL.md). You now have `junkcoind` or `junkcoin-qt` executables available to run a node, and `junkcoin-cli`/`junkcoin-tx` tools to help you transact DOGE.
...
The Dogecoin Core installation provides the `junkcoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.
